...
#-------------------------ADD RECORDING-------------------------#
    def add_record(self, patient_id):
        ...
        self.ui.startrec_btn.clicked.connect(self.start_recording(patient_id))
    ...
    def analyze_save_recording(self, patient_id):
        ...
        if recording_file:
            ...
            patient_rec_ref = db.reference('patients').child(patient_id).child('recordings')
            ...
            if user:
                ...
                recordings_ref = db.reference('patients').child(patient_id).child('recordings').child(str(datetime.datetime.now()))
                phrase = self.ui.recphrase.text()
                notes = self.ui.recnotes.text()                                                                          

                # Update progress record in the database
                patient_ref = db.reference(f'patients/{patient_id}') #UPDATE THIS
    ...
    def start_recording(self, patient_id):
        pass
...
#--------------------------ADD PATIENTS--------------------------#
    ...
    def add_patients(self):
        ...
        if user:
            ...
            doctor_ref = db.reference('doctors').child(user_id).child('patients').child(patient_id)
            doctor_ref.set('')
